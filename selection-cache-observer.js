(() => {
  ...
  function collectModernIds(stableIds) {
    ...
    for (const exchange of exchanges) {
      ...
      if (userId && userId !== 'client-created-root' && !isTemporaryId(userId)) {
        stableIds.add(userId);
      }

      const assistant = exchange.querySelector('[data-chatgpt-selection-message-id]');
      const assistantId = assistant?.getAttribute('data-chatgpt-selection-message-id') || null;
      if (assistantId && assistantId !== 'client-created-root' && !isTemporaryId(assistantId)) {
        stableIds.add(assistantId);
      }
    }
  ...
  function collectIds() {
    ...
    // Current ChatGPT DOM exposes stable user and assistant message UUIDs
    // directly on each exchange. Prefer that representation when present.
    const modern = collectModernIds(stableIds);
    ...
    if (!modern) {
      ...
      // Defensive fallback for a future legacy DOM variant where mounted turns
      // are no longer wrapped in data-turn-id-container. As above, a stable
      // message UUID wins over the surrounding turn ID.
